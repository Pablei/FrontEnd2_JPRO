let tableClientes = document.querySelector("#table-clientes > tbody");
let searchInput = document.querySelector("#search-input");

// Evento para probar el campo de buscar
searchInput.addEventListener("keyup", () => {
    console.log(searchInput.value);
});

// Evento para el navegador
document.addEventListener("DOMContentLoaded", () => {
    getTableData();
});

// Función para traer los datos de la base de datos a la tabla
let getTableData = async () => {
    let url = "http://localhost/backend-apicrud/index.php?url=clientes";
    try {
        let respuesta = await fetch(url, {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
        });

        if (respuesta.status === 204) {
            console.log("No hay datos en la BD");
        } else {
            let tableData = await respuesta.json();
            console.log(tableData);
            //Agregar los datos de la tabla a localStorage
            localStorage.setItem("datosTabla", JSON.stringify(tableData));

            // Agregar los datos a la tabla
            tableData.forEach((dato, i) => {
                let row = document.createElement("tr");
                row.innerHTML = `
                    <td>${i + 1}</td>
                    <td>${dato.nombre}</td>
                    <td> ${dato.apellido} </td>
                    <td>${dato.email} </td>
                    <td>${dato.celular} </td>
                    <td>${dato.direccion} </td>
                    <td> ${dato.direccion2}  </td>
                    <td>${dato.descripcion}  </td>
                    <td>
                    <button id="btn-edit" onclick="editDataTable(${i})"  type="button" class="btn btn-warning"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
  <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
  <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
</svg>
                    
                    </button> 
                     <button id="btn-delete" onclick="deleteDataTable(${i})" type="button" class="btn btn-danger"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
  <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5M11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47M8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5"/>
</svg>
                     
                     </button>  
                     <button id="btn-visualize" onclick="showDataTable(${i})" type="button" class="btn btn-info"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye" viewBox="0 0 16 16">
  <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8M1.173 8a13 13 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5s3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5s-3.879-1.168-5.168-2.457A13 13 0 0 1 1.172 8z"/>
  <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5M4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0"/>
</svg>
                     
                     </button>
                    </td>
                `;
                tableClientes.appendChild(row);
            });
        }
    } catch (error) {
        console.log(error);
    }
};

//Funcion apra editar algun cliente en la tabla

let editDataTable = (pos)=>{
let clients = [];
let clientsSave = JSON.parse(localStorage.getItem("datosTabla"));
if (clientsSave != null){
    clients = clientsSave;
}
let singleClient = clients [pos];

//console.log (singleClient);
localStorage.setItem ("clientEdit", JSON.stringify(singleClient));
localStorage.removeItem("datosTabla");
location.href= "../crear-cliente.html"
} 







//Funcion para eliminar algun dato de la tabla

let deleteDataTable = (pos)=>{
    let clients = [];
let clientsSave = JSON.parse(localStorage.getItem("datosTabla"));
if (clientsSave != null){
    clients = clientsSave;
}
let singleClient = clients [pos];
//console.log("Cliente a eliminar: "+ singleClient.nombre);
let IDclient= {
    id: singleClient.id_cliente
}
let confirmar = confirm (`Deseas eliminar ${singleClient.nombre} ?`);
if (confirmar){
    //llamar la funcion para realizar la peticion
    sendDeleteClient(IDclient);
}
    
}
//funcion para realizar la peticion de eliminar un producto
let sendDeleteClient = async (id) => {
    let url = "http://localhost/backend-apicrud/index.php?url=clientes";
    try {
        let respuesta = await fetch(url, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify(id)
        });

        if (!respuesta.ok) {
            let errorText = await respuesta.text();
            console.error("Error del servidor:", errorText);
            alert("No se pudo eliminar el cliente.");
            return;
        }

        let Mensaje = await respuesta.json();
        alert(Mensaje.message);
        location.reload();

    } catch (error) {
        console.log("Error en la petición:", error);
    }
}








//Funcion para visualizar los datos de las tablas
let showDataTable = (pos) => {
    let clients = JSON.parse(localStorage.getItem("datosTabla")) || [];
    let client = clients[pos];

    // Rellenar los campos del modal
    document.getElementById("modal-nombre-completo").textContent = client.nombre || "—";
    document.getElementById("modal-email").textContent = client.email || "—";
    document.getElementById("modal-celular").textContent = client.celular || "—";
    document.getElementById("modal-direccion").textContent = client.direccion || "—";
    document.getElementById("modal-direccion2").textContent = client.direccion2 || "—";
    document.getElementById("modal-descripcion").textContent = client.descripcion || "—";

    // Mostrar el modal (usando Bootstrap)
    $('#verClienteModal').modal('show');
};
